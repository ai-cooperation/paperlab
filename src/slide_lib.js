...
function conceptSlide(pres, title, bigText, desc, items) {
  ...
  s.addText(desc, { x: 0.6, y: 1.55, w: 8.8, h: 0.5, fontSize: 12, color: C.grayText, align: "center", valign: "middle" });
  // Bullet items
  if (items && items.length > 0) {
    const fs = items.length > 5 ? 10 : 11;
...
function stepSlide(pres, seriesTitle, stepNum, stepTitle, lines) {
  ...
  s.addText(`Step ${stepNum}`, { x: 0.3, y: 0.9, w: 0.65, h: 0.65, fontSize: 10, bold: true, color: C.white, align: "center", valign: "middle" });
  s.addText(stepTitle, { x: 1.1, y: 0.92, w: 8.5, h: 0.6, fontSize: 20, bold: true, color: C.darkText, valign: "middle" });
  // Content lines
  const fs = lines.length > 5 ? 10.5 : 11.5;
...
function exampleSlide(pres, title, scenario, resultLines) {
  ...
  s.addText("情境：", { x: 0.5, y: 0.88, w: 0.8, h: 0.35, fontSize: 11, bold: true, color: C.orange, valign: "middle" });
  s.addText(scenario, { x: 1.3, y: 0.88, w: 8.2, h: 0.75, fontSize: 11, color: C.darkText, valign: "middle", lineSpacingMultiple: 1.2 });
  // Result
  s.addText("AI 回覆：", { x: 0.3, y: 1.85, w: 2, h: 0.35, fontSize: 12, bold: true, color: C.teal });
  s.addShape(pres.shapes.ROUNDED_RECTANGLE, { x: 0.3, y: 2.2, w: 9.4, h: 3.0, fill: { color: C.lgrey }, rectRadius: 0.06 });
  const fs = resultLines.length > 8 ? 9.5 : 10.5;
...
function exerciseSlide(pres, title, instructions, timeStr) {
  ...
  if (timeStr) s.addText(timeStr, { x: 2.0, y: 0.76, w: 5, h: 0.3, fontSize: 10, color: C.grayText });
  instructions.forEach((inst, i) => {
    ...
    s.addText(inst, { x: 0.85, y, w: 8.8, h: 0.45, fontSize: 11.5, color: C.darkText, valign: "middle" });
  });
  s.addShape(pres.shapes.RECTANGLE, { x: 0, y: 5.4, w: 10, h: 0.225, fill: { color: C.navy } });
  return s;
}
...
function compareSlide(pres, title, lTitle, lItems, rTitle, rItems) {
  ...
  lItems.forEach((item, i) => {
    s.addText("\u2022 " + item, { x: 0.6, y: 1.5 + i * 0.42, w: 4.0, h: 0.38, fontSize: 10.5, color: C.darkText, valign: "middle" });
  });
  // Right column
  ...
  rItems.forEach((item, i) => {
    s.addText("\u2022 " + item, { x: 5.5, y: 1.5 + i * 0.42, w: 4.0, h: 0.38, fontSize: 10.5, color: C.darkText, valign: "middle" });
  });
  return s;
}
...
function cardsSlide(pres, title, cards) {
  ...
  cards.forEach((card, i) => {
    ...
    s.addText(card.title, { x: x + 0.1, y: ty, w: cw - 0.2, h: 0.4, fontSize: 12, bold: true, color: accents[i % 4], align: "center" });
    // Description
    const dy = ty + 0.45;
    const desc = Array.isArray(card.desc) ? card.desc.join("\n") : card.desc;
    s.addText(desc, { x: x + 0.12, y: dy, w: cw - 0.24, h: 4.2 - (dy - 0.9) - 0.15, fontSize: 10, color: C.darkText, lineSpacingMultiple: 1.25 });
  });
  return s;
}
...
function keyPointsSlide(pres, title, points) {
  ...
  points.forEach((pt, i) => {
    ...
    // Description
    s.addText(pt.desc, { x: x + 0.55, y: y + 0.35, w: colW - 0.7, h: rowH - 0.42, fontSize: 10, color: C.grayText, lineSpacingMultiple: 1.2 });
  });
  return s;
}
...
function outputSlide(pres, title, deliverables) {
  ...
  deliverables.forEach((d, i) => {
    ...
    s.addText(d.item, { x: 0.5, y, w: 3.5, h: 0.65, fontSize: 12, bold: true, color: C.navy, valign: "middle" });
    s.addText(d.desc, { x: 4.0, y, w: 5.5, h: 0.65, fontSize: 10.5, color: C.darkText, valign: "middle" });
  });
  return s;
}
...
function beforeAfterSlide(pres, title, beforeLines, afterLines) {
  ...
  beforeLines.forEach((line, i) => {
    s.addText(line, { x: 0.6, y: 1.45 + i * 0.38, w: 4.0, h: 0.34, fontSize: 10, color: C.darkText, valign: "middle" });
  });
  // After
  s.addShape(pres.shapes.ROUNDED_RECTANGLE, { x: 5.2, y: 0.9, w: 4.5, h: 4.2, fill: { color: C.greenBg }, rectRadius: 0.08 });
  s.addText("After", { x: 5.4, y: 1.0, w: 2.5, h: 0.35, fontSize: 14, bold: true, color: C.green });
  afterLines.forEach((line, i) => {
    s.addText(line, { x: 5.5, y: 1.45 + i * 0.38, w: 4.0, h: 0.34, fontSize: 10, color: C.darkText, valign: "middle" });
  });
  return s;
}
...
function tocSlide(pres, title, items) {
  ...
  items.forEach((item, i) => {
    ...
    s.addText(item.time || "", { x: 7.0, y, w: 2.7, h: 0.3, fontSize: 10, color: C.grayText, valign: "middle", align: "right" });
    if (item.desc) s.addText(item.desc, { x: 1.05, y: y + 0.3, w: 8.5, h: 0.3, fontSize: 10, color: C.grayText, valign: "middle" });
  });
  return s;
}
...
function moduleSection(pres, secNum, secTitle, timeStr, cards) {
  ...
  s.addText(timeStr, { x: 0.4, y: 0.72, w: 5, h: 0.3, fontSize: 10, color: C.grayText });
  const nc = Math.min(cards.length, 4);
  const gap = 0.15;
  const cw = (9.4 - gap * (nc - 1)) / nc;
  cards.forEach((card, i) => {
    ...
    s.addText(card.title, { x: x + 0.1, y: 1.35, w: cw - 0.2, h: 0.4, fontSize: 12, bold: true, color: C.navy });
    s.addText(card.desc, { x: x + 0.1, y: 1.8, w: cw - 0.2, h: 3.1, fontSize: 10, color: C.darkText, lineSpacingMultiple: 1.3 });
  });
  return s;
}
...
function infoCard(s, pres, x, y, w, h, title, body, accent = C.teal) {
  ...
  s.addText(body, { x: x + 0.15, y: y + 0.46, w: w - 0.22, h: h - 0.56, fontSize: 10.5, color: C.darkText, margin: 0, lineSpacingMultiple: 1.2, fit: "shrink" });
}
...
function codeBox(s, pres, x, y, w, h, title, code) {
  ...
  s.addText(code, { x: x + 0.12, y: cy, w: w - 0.24, h: ch, fontSize: 9, fontFace: "Consolas", color: C.teal, margin: 0, lineSpacingMultiple: 1.25, fit: "shrink" });
}
...
function warnBox(s, x, y, w, text) {
  ...
  s.addText(text, { x: x + 0.2, y, w: w - 0.3, h: 0.55, fontSize: 11, bold: true, color: C.red, valign: "middle" });
}
