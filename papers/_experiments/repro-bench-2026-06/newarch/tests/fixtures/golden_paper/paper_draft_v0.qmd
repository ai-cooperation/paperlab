...
**Background.** Exercise interventions are widely recognised as beneficial for mental health in older adults, but whether abstract-level data — extracted solely from titles and abstracts indexed in OpenAlex — can recover a directionally informative pooled estimate remains untested in this domain.

**Methods.** We conducted a DerSimonian-Laird random-effects meta-analysis with abstract-level, pattern-based mechanical extraction of effect sizes from OpenAlex metadata and EuropePMC batch retrieval. Screening followed a single-reviewer PICOS-based protocol applied to abstracts without full-text retrieval. Effect sizes were extracted as standardised mean differences (SMD), raw mean differences (MD), and log-scale ratios from abstract-reported statistics. Heterogeneity was quantified via Cochran's Q, I-squared, and tau-squared. All pooling was performed separately by effect-size scale; MD values were reported per-study but not pooled because the original-unit scales were heterogeneous and unverifiable from abstracts. Egger's regression test and leave-one-out sensitivity analysis were performed. No Bayesian, network, or meta-regression models were applied.

**Results.** From 66,156 records identified, 2,400 abstracts were screened; 24 studies reported extractable effects, and 64 effects were extracted. The SMD pool included k = 8 effects and the log-ratio pool included k = 3 effects. The pooled SMD was −0.4327 (95% CI: −0.9057 to 0.0402), indicating a reduction in depressive symptoms favouring exercise with the confidence interval crossing zero. The pooled log-ratio estimate was 1.55 (95% CI: 0.5988 to 4.0121). Heterogeneity was considerable for both scales (SMD: I² = 95.4%, τ² = 0.4254, Q = 150.871; log-ratio: I² = 95.5%, τ² = 0.6415, Q = 44.72). Egger's test showed no strong funnel asymmetry (intercept = −8.152, t = −1.776, df = 6).

**Conclusion.** Abstract-level pooling recovers a directionally consistent signal for exercise effects on depressive symptoms in older adults, supporting its use in rapid-evidence surveillance contexts, though the wide uncertainty interval and absence of risk-of-bias assessment preclude clinical recommendation.
