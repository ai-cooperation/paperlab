...
def test_build_canonical_data_v3_2_recomputes_rate_from_verified_rows_over_bad_summary(tmp_path: Path):
    run_dir = tmp_path / "run"
    run_dir.mkdir()
    (run_dir / "references.bib").write_text(
        "\n".join(
            "@article{ref%s,title={T%s},doi={10.1000/%s}}" % (idx, idx, idx)
            for idx in range(40)
        ),
        encoding="utf-8",
    )
        ...
        json.dumps(
            {
                ...
                "doi_real_rate": 0.0,
                "records": [
                    {"doi": "10.1000/%s" % idx, "validation_count": 3}
                    for idx in range(40)
                ],
            }
        ),
...
def test_build_canonical_data_v3_2_accepts_nested_valid_source_rows(tmp_path: Path):
    ...
    (run_dir / "doi_audit.json").write_text(
        json.dumps(
            {
                "summary": {"doi_real_rate": 0.0},
                "entries": [
                    {
                        "doi": "10.1000/%s" % idx,
                        "crossref": {"checked": True, "valid": True},
                        "openalex": {"checked": True, "valid": True},
                        "semantic_scholar": {"checked": False, "valid": None},
                    }
                    for idx in range(40)
                ],
            }
...
def test_top_up_references_from_contract_appends_verified_seed_refs(tmp_path: Path):
    run_dir = tmp_path / "run"
    run_dir.mkdir()
    (run_dir / "references.bib").write_text(
        "\n".join("@article{base%s,title={Base %s},doi={10.1000/base.%s}}" % (idx, idx, idx) for idx in range(33)),
        encoding="utf-8",
    )
    ...
    result = top_up_references_from_contract_v3_2(run_dir, floor=35)

    assert result["status"] == "done"
    assert result["before_count"] == 33
    assert result["after_count"] == 35
...
def test_validate_data_outputs_v3_2_reports_missing_declared_outputs(tmp_path: Path):
    run_dir = tmp_path / "run"
    run_dir.mkdir()
    (run_dir / "references.bib").write_text("@article{one,title={One}}\n", encoding="utf-8")
    (run_dir / "doi_audit.json").write_text("{}", encoding="utf-8")

    result = validate_data_outputs_v3_2(
        run_dir,
        [
            "references.bib",
            "doi_audit.json",
    ...
    assert "real_experiments/real_results.json" in result["missing_outputs"]
    assert "figures/fig_prisma_flow.png" in result["missing_outputs"]


def test_run_data_harness_v3_2_writes_topup_canonical_and_completeness(tmp_path: Path):
    run_dir = tmp_path / "run"
    (run_dir / "real_experiments").mkdir(parents=True)
    (run_dir / "figures").mkdir()
    (run_dir / "references.bib").write_text(
        "\n".join("@article{base%s,title={Base %s},doi={10.1000/base.%s}}" % (idx, idx, idx) for idx in range(34)),
        encoding="utf-8",
    )
    (run_dir / "doi_audit.json").write_text(
        json.dumps({"records": [{"doi": "10.1000/base.%s" % idx, "validation_count": 2} for idx in range(34)]}),
        encoding="utf-8",
    )
    (run_dir / "research_contract.input.json").write_text(
        json.dumps({"literature": {"verified_refs": [{"key": "seedC", "doi": "10.1000/seed.c", "title": "Seed C"}]}}),
        encoding="utf-8",
    )
    (run_dir / "real_experiments" / "real_results.json").write_text(
        json.dumps({"effects": [{"id": "a"}]}),
        encoding="utf-8",
    )
    for name in ("fig_prisma_flow", "fig_method_overview", "fig_forest_plot", "fig_benchmark_comparison"):
        (run_dir / "figures" / ("%s.svg" % name)).write_text("<svg/>", encoding="utf-8")
        (run_dir / "figures" / ("%s.png" % name)).write_bytes(b"png")

    result = run_data_harness_v3_2(
        run_dir,
        [
    ...
    assert result["topup"]["after_count"] == 35
    assert result["completeness"]["status"] == "done"
    assert (run_dir / CANONICAL_DATA_V3_2_PATH).is_file()
    canonical = json.loads((run_dir / CANONICAL_DATA_V3_2_PATH).read_text(encoding="utf-8"))
    assert canonical["references"]["count"] == 35
...
def test_build_data_substeps_v3_2_reports_deterministic_data_loop(tmp_path: Path):
    ...
    (run_dir / REFERENCE_TOPUP_V3_2_PATH).write_text(
    ...
    (run_dir / "figures" / "fig_prisma_flow.svg").write_text("<svg/>", encoding="utf-8")
    (run_dir / "figures" / "fig_prisma_flow.png").write_bytes(b"png")
    (run_dir / DATA_COMPLETENESS_V3_2_PATH).write_text(
        json.dumps({"status": "done", "missing_outputs": [], "invalid_outputs": []}),
        encoding="utf-8",
    )
    ...
    assert {step["id"]: step["status"] for step in substeps} == {
        ...
        "top_up_references": "done",
        "extract_abstract_level_effects": "done",
        "write_canonical_data": "done",
        "generate_figures": "done",
        "data_output_completeness": "done",
        "gate_A_E_G": "pending",
    }
    assert substeps[2]["owner"] == "deterministic"
    assert substeps[3]["owner"] == "deterministic"
    assert substeps[4]["owner"] == "hermes_bounded"
