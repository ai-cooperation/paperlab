...
def normalize_frontmatter(run_dir: Path, contract: dict[str, Any], src_name: str = "paper_draft_v0.qmd",
    ...
    new_fm = (
        ...
        "        % round for this). A 7-column longtable header cell overflows the narrow\n"
        "        % p{} column by ~15pt under \\footnotesize; the document-global\n"
        "        % \\emergencystretch pandoc injects cannot absorb it because a single\n"
        "        % oversized header cell has no interword glue to stretch. Scope the fix\n"
        "        % to the longtable environment (auto-reverts on \\end{longtable}, so body\n"
        "        % paragraph spacing is untouched): drop the font one more step to\n"
        "        % \\scriptsize (narrows the header text), turn on \\sloppy (permits looser\n"
        "        % breaks inside cells), and re-arm \\emergencystretch so residual\n"
        "        % overflow is soaked up instead of reaching Gate Z.\n"
        "        \\AtBeginEnvironment{longtable}{\\singlespacing\\scriptsize\\sloppy\\setlength{\\emergencystretch}{3em}}\n"
        "        \\AtBeginEnvironment{tabular}{\\singlespacing\\footnotesize\\sloppy\\setlength{\\emergencystretch}{3em}}\n"
